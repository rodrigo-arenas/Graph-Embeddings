...
def test_node2vec_properties(graph):
    ...
    assert len(embeddings_model.unique_nodes) == 4
    assert embeddings_model.embeddings.shape == (4, 16, 2)  # nodes, embedding_size, graphs
    assert embeddings_model.dense_embeddings.shape == (2, 16 * 4)  # graphs, embedding_size*nodes
